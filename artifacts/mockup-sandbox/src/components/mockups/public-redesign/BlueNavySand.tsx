...
export function BlueNavySand() {
  ...
  return (
    <div className="min-h-screen bg-[#1E3A6E] font-sans selection:bg-[#5B9FE0] selection:text-white overflow-x-hidden">
      ...
      {/* Hero Section */}
      <section className="relative hero-bg-light pt-32 pb-20 lg:pt-48 lg:pb-32 overflow-hidden">
        {/* Decorative background shape */}
        <div className="absolute top-20 right-20 w-[600px] h-[600px] border border-[#5B9FE0]/20 rounded-full floating-shape pointer-events-none"></div>
        <div className="absolute top-40 right-40 w-[400px] h-[400px] border border-[#5B9FE0]/30 rounded-full floating-shape pointer-events-none" style={{ animationDelay: '-5s' }}></div>

        <div className="container mx-auto px-6 relative z-10">
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div className="reveal">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#F2E8D5] text-[#1E3A6E] text-xs font-bold uppercase tracking-wider mb-6">
              ...
              <h1 className="text-5xl lg:text-6xl xl:text-7xl font-black text-white leading-[1.1] tracking-tight mb-6 font-['Barlow_Condensed'] uppercase">
                Two Teams.<br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-white to-[#BFD9F5]">One Flag.</span><br />
                One World Cup.
              </h1>
              
              <p className="text-xl text-[#BFD9F5] font-medium mb-4 max-w-lg">
                Hong Kong's finest veteran players take on the world stage.
              </p>
              <p className="text-[#8FBDE8] mb-10 max-w-lg text-lg leading-relaxed">
              ...
              <div className="flex flex-wrap gap-4">
                ...
                <button className="px-8 py-4 bg-transparent border-2 border-[#F2E8D5]/40 hover:border-[#F2E8D5] text-[#F2E8D5] font-bold rounded-lg transition-colors">
                  About HK Masters
                </button>
              </div>
            </div>
            ...
            <div className="reveal relative lg:ml-auto w-full max-w-md" style={{ animationDelay: '0.2s' }}>
              <div className="aspect-[4/5] rounded-2xl bg-gradient-to-br from-[#2A5298] to-[#5B9FE0]/70 p-1 relative overflow-hidden shadow-2xl shadow-[#1A2E55]/40 group">
                <div className="absolute inset-0 bg-white/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                <div className="absolute inset-0 rounded-2xl shadow-[inset_0_0_40px_rgba(255,255,255,0.12)] pointer-events-none"></div>
                <div className="w-full h-full rounded-xl border border-white/15 flex items-center justify-center relative overflow-hidden">
                  <svg className="w-3/4 h-3/4 opacity-25 text-white" viewBox="0 0 100 100" fill="currentColor">
      ...
      {/* Photo Strip & Stats */}
      <section className="bg-[#16305D] py-6 border-y border-white/10 relative z-20">
        <div className="container mx-auto px-6">
          <div className="flex flex-nowrap overflow-x-auto gap-4 pb-4 snap-x no-scrollbar">
            {[1, 2, 3, 4, 5, 6, 7].map((i) => (
              <div key={i} className="min-w-[120px] w-[120px] h-[80px] rounded-lg bg-[#5B9FE0]/20 border border-[#5B9FE0]/35 snap-center shrink-0 overflow-hidden relative group cursor-pointer">
                <div className="absolute inset-0 bg-[#5B9FE0] mix-blend-overlay opacity-40 group-hover:opacity-0 transition-opacity"></div>
              </div>
            ))}
          </div>
          
          <div className="grid grid-cols-3 gap-4 pt-6 border-t border-[#5B9FE0]/25 text-center">
            <div className="reveal" style={{ animationDelay: '0.1s' }}>
              <div className="text-3xl font-black text-white mb-1">2</div>
              <div className="text-xs uppercase tracking-widest text-[#BFD9F5] font-bold">Squads</div>
            </div>
            <div className="reveal border-l border-[#5B9FE0]/25" style={{ animationDelay: '0.2s' }}>
              <div className="text-3xl font-black text-white mb-1">Rotterdam</div>
              <div className="text-xs uppercase tracking-widest text-[#BFD9F5] font-bold">2026 Host</div>
            </div>
            <div className="reveal border-l border-[#5B9FE0]/25" style={{ animationDelay: '0.3s' }}>
              <div className="text-3xl font-black text-white mb-1">Jul-Aug</div>
              <div className="text-xs uppercase tracking-widest text-[#BFD9F5] font-bold">22 Jul – 1 Aug</div>
            </div>
          </div>
        </div>
      </section>
      ...
      {/* Countdown Section */}
      <section className="bg-[#2A5298] py-24 relative overflow-hidden">
        {/* Decorative background grid */}
        <div className="absolute inset-0 bg-[linear-gradient(rgba(191,217,245,0.06)_1px,transparent_1px),linear-gradient(90deg,rgba(191,217,245,0.06)_1px,transparent_1px)] bg-[size:40px_40px] pointer-events-none"></div>

        <div className="container mx-auto px-6 relative z-10 text-center reveal">
          <div className="inline-flex items-center justify-center px-4 py-1.5 rounded-full bg-[#5B9FE0] text-white text-xs font-bold uppercase tracking-widest mb-6">
            Rotterdam 2026 Masters World Cup
          </div>
          ...
          <div className="flex flex-wrap justify-center items-center gap-4 md:gap-8 mb-12">
            {[
              ...
              <React.Fragment key={unit.label}>
                <div className="bg-white/10 border border-[#BFD9F5]/40 rounded-2xl p-6 min-w-[120px] md:min-w-[140px] backdrop-blur-sm shadow-xl">
                  <div className="text-5xl md:text-6xl font-extrabold text-white mb-2 font-['Barlow_Condensed']">{unit.value}</div>
                  <div className="text-xs md:text-sm font-bold text-[#BFD9F5] tracking-widest uppercase">{unit.label}</div>
                </div>
                {i < arr.length - 1 && (
                  <div className="hidden md:block text-4xl font-black text-[#BFD9F5]/50">:</div>
                )}
              </React.Fragment>
            ))}
          </div>
          
          <p className="text-[#BFD9F5] font-medium text-lg mb-8">Opening Match: July 22, 2026</p>
      ...
      <section className="bg-[#F2E8D5] py-24 text-[#292320]">
        <div className="container mx-auto px-6">
          <div className="text-center max-w-2xl mx-auto mb-16 reveal">
            <h2 className="text-4xl md:text-5xl font-bold text-[#1E3A6E] mb-6 font-['Barlow_Condensed'] uppercase">About the Tournament</h2>
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {[
              ...
              <div key={squad.id} className="bg-white rounded-2xl p-8 shadow-md border border-[#EDE0C4] hover:-translate-y-2 hover:shadow-xl transition-all duration-300 group reveal" style={{ animationDelay: `${i * 0.2}s` }}>
                <div className="inline-block px-3 py-1 rounded bg-[#2A5298] text-white text-sm font-bold mb-6">
                  {squad.id}
                </div>
                <h3 className="text-2xl font-bold text-[#1E3A6E] mb-4">{squad.title}</h3>
                ...
                <a href="#" className="inline-flex items-center gap-2 text-[#2A5298] font-bold group-hover:gap-3 transition-all">
                  View Squad
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                    <path d="M5 12H19M19 12L12 5M19 12L12 19" strokeLinecap="round" strokeLinejoin="round"/>
                  </svg>
      ...
      {/* Footer */}
      <footer className="bg-[#16305D] text-white py-12 border-t-4 border-[#5B9FE0]">
        <div className="container mx-auto px-6 text-center">
          ...
          <div className="w-24 h-px bg-[#2A5298] mx-auto mb-6"></div>
          <p className="text-[#8FBDE8] text-sm">
