  ...
  if (cta) {
    return (
      <Link
        href={href}
        onClick={onClick}
        className={`btn-shimmer inline-flex items-center text-sm font-bold px-4 py-1 rounded-md transition-colors duration-150 ${
  ...
  return (
    <Link
      ...
      className={`nav-link text-sm font-medium whitespace-nowrap transition-colors duration-150 hover:text-white ${
        isActive ? "text-white active" : "text-[#BFD9F5]"
      }`}
    >
      {label}
    </Link>
...
export default function Layout({ children }) {
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      el.style.transform = `scaleX(${Math.min(pct || 0, 1)})`;
    };
    ...
    <div className="min-h-screen flex flex-col">
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between h-[68px]">
              ...
              <Link href="/" className="flex items-center gap-3 group shrink-0">
                <div className="h-12 w-12 rounded-lg bg-[#1E3A6E] shrink-0 overflow-hidden flex items-center justify-center shadow-sm">
                  <img
                ...
                <div className="leading-tight">
                  <p className="text-[#1E3A6E] font-extrabold text-[17px] leading-tight tracking-tight">
                    Hong Kong<br />Masters Hockey
                  </p>
                  <p className="text-[#5B9FE0] text-[9px] font-bold uppercase tracking-[0.18em] mt-0.5">
                    Hockey · Rotterdam 2026
                  </p>
                </div>
              </Link>
              ...
              {/* Mobile hamburger */}
              <button
                className="lg:hidden p-2 text-[#1E3A6E] focus:outline-none"
      ...
      <footer className="bg-[#1E3A6E] text-white mt-auto">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div>
              <div className="flex items-start gap-3 mb-3">
                <div className="h-12 w-12 rounded-lg bg-[#F2E8D5] shrink-0 overflow-hidden flex items-center justify-center">
                  <img
                    src={`${import.meta.env.BASE_URL}logo.png`}
                    alt="HK Masters Hockey logo"
                    className="w-full h-full object-contain scale-150"
              ...
              <div className="grid grid-cols-2 gap-x-4">
                {[NAV_LINKS.slice(0, 5), NAV_LINKS.slice(5)].map((col, ci) => (
                  <ul key={ci} className="space-y-1">
                    {col.map((link) => (
                      <li key={link.href}>
                        <Link href={link.href} className="text-[#8FBDE8] text-sm hover:text-white transition-colors duration-150">
                          {link.label}
                        </Link>
                      </li>
                    ))}
            ...
            <div>
              ...
              <a href={`mailto:${contactContent.email}`} className="text-[#8FBDE8] text-sm hover:text-white transition-colors duration-150 block mb-3">
                {contactContent.email}
              </a>
              <div className="flex gap-3">
                {contactContent.social.facebook && (
                  <a href={contactContent.social.facebook} target="_blank" rel="noopener noreferrer" aria-label="Facebook" className="text-[#8FBDE8] hover:text-white transition-colors duration-150">
                ...
                {contactContent.social.instagram && (
                  <a href={contactContent.social.instagram} target="_blank" rel="noopener noreferrer" aria-label="Instagram" className="text-[#8FBDE8] hover:text-white transition-colors duration-150">
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                ...
                {contactContent.social.twitter && (
                  <a href={contactContent.social.twitter} target="_blank" rel="noopener noreferrer" aria-label="Twitter/X" className="text-[#8FBDE8] hover:text-white transition-colors duration-150">
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                ...
                {contactContent.social.youtube && (
                  <a href={contactContent.social.youtube} target="_blank" rel="noopener noreferrer" aria-label="YouTube" className="text-[#8FBDE8] hover:text-white transition-colors duration-150">
                    <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div className="mt-8 pt-6 border-t border-[#2A5298] text-center">
            <p className="text-[#F2E8D5] font-bold italic text-base mb-2 tracking-wide">— Hockey For Life —</p>
