...
export function Scene3() {
  ...
  return (
    <motion.div
      ...
      style={{ background: 'var(--iv-bg-dark)' }}
      initial={{ x: '100%' }}
      animate={{ x: 0 }}
      exit={{ x: '-100%' }}
            ...
            {phase < 3 ? (
              <div className="relative w-full h-full bg-white flex flex-col">
                ...
                <div className="flex-1 bg-gray-50 p-3 opacity-30 overflow-hidden">
                  <div className="w-full h-28 bg-[#1E3A6E] rounded-xl mb-3" />
                ...
                <div className="absolute inset-x-0 bottom-0 bg-[#F2F2F7] rounded-t-3xl shadow-[0_-8px_32px_rgba(0,0,0,0.18)] z-10">
                  ...
                  <div className="flex gap-4 px-5 mb-4 overflow-hidden">
                    {['Message', 'Mail', 'Notes', 'Copy'].map((label, i) => (
                      <div key={i} className="flex flex-col items-center gap-1.5 shrink-0">
                        <div className={`w-12 h-12 rounded-2xl flex items-center justify-center shadow-sm ${['bg-green-500', 'bg-blue-500', 'bg-yellow-400', 'bg-gray-400'][i]}`}>
                  ...
                  <div className="mx-4 bg-white rounded-2xl overflow-hidden shadow-sm mb-3">
                    {[{ icon: '🔖', label: 'Add Bookmark' }, { icon: '📋', label: 'Copy Link' }].map(({ icon, label }) => (
                    ...
                    >
                      ...
                      {phase >= 2 && (
                        <motion.div className="absolute inset-0 bg-blue-100" initial={{ scaleX: 0, transformOrigin: 'left' }} animate={{ scaleX: 1 }} transition={{ duration: 0.35 }} style={{ zIndex: -1, opacity: 0.6 }} />
                      )}
                    </motion.div>
                  </div>
              ...
              <motion.div className="w-full h-full" style={{ background: 'linear-gradient(160deg, #1a3a6b 0%, #0d2445 60%, #071629 100%)' }} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
                  ...
                  <div className="flex flex-wrap gap-3">
                    {[...Array(8)].map((_, i) => (
                      <div key={i} className="w-[52px] h-[52px] flex flex-col items-center gap-1">
                        <div className="w-12 h-12 rounded-2xl" style={{ background: 'rgba(255,255,255,0.08)' }} />
                        <div className="w-10 h-1.5 bg-white/20 rounded" />
                      </div>
                    ))}
                    {phase >= 4 && (
                      <motion.div className="flex flex-col items-center gap-1" initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', stiffness: 350, damping: 18 }}>
                        <div className="w-12 h-12 rounded-2xl bg-[#1E3A6E] border-2 border-[rgba(242,232,213,0.6)] flex items-center justify-center shadow-lg shadow-black/50 overflow-hidden">
